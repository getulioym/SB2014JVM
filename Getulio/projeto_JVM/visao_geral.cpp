...
int main(int argc, char **argv) {
	...
	//inicializa os campos static se tiver
	metodo = memoria.pega_metodo("<cinit>"); // memoria.classes.pega_metodo("<cinit>")
	if( metodo <cinit> existe) {
		code = metodo.pega_code();
		param = metodo.pega_param();
		frames.cria_frame(code, param);
		while(!frames.esta_vazia()) {
			byte_code = frames.pega_byte_code();  // frames.code[frames.PC]
			opcode(byte_code);
		}
	}
